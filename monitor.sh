...
journalctl -f | while read -r line; do
    case "$line" in
        *"Failed password"*)
        ...
        JSON_STRING=$(jq -n --arg login "ssh-failed" '$ARGS.named' --arg ipaddress "$IP" 'ARGS.named')
        ;;

        *"incorrect password"*)
        echo "Failed sudo attempt detected: $line"
        JSON_STRING=$(jq -n --arg login "sudo-failed" '$ARGS.named' --arg log "$line" 'ARGS.named')
        ;;

        *COMMAND*etc*)
        echo "Watched file edited $line"
        JSON_STRING=$(jq -n --arg login "file-edited" '$ARGS.named' --arg log "$line" 'ARGS.named')
        ;;
    esac
